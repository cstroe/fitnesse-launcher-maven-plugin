...
public class WikiMojoTest {
	...
	static int PORT = FitNesseHelper.DEFAULT_COMMAND_PORT;
	...
	public void setUp() throws IOException, PluginException {
		...
		//when(fitNesseHelper.findFitNesseServerThread()).thenCallRealMethod();
	}
	...
	public void testWikiMojoBasic() throws Exception {
		...
		verify(fitNesseHelper, times(1)).launchFitNesseServer(PORT, mojo.workingDir, mojo.root, mojo.logDir);
		verify(fitNesseHelper, never()).createSymLink(any(File.class), anyString(), anyInt(), any(Launch.class));
		verify(fitNesseHelper, times(1)).shutdownFitNesseServer(PORT);
	...
	public void testWikiLaunchException() throws Exception {
		doThrow(new IOException("TEST")).when(fitNesseHelper).launchFitNesseServer(anyInt(), anyString(), anyString(), anyString());
